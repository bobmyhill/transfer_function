...
The current model is used to approximate the displacement field of
an elastic block when it is deformed by
small surface loads imposed by circular feet.

This is a decent approximation to the behaviour of the InSight lander
on Mars
"""
...
# distance_from_origin = 1.221 # m; Murdoch et al., 2018
#foot_centers = np.array([[-1.221, 0., H/2.],
#                         [0.610, -1.057, H/2.],
#                         [0.610, 1.057, H/2.]])
...
try:
    ...
    #refinement_radii = [lander_foot_radius*8.] # single refinement for testing purposes
...
# Boundary conditions with no slip base and free slip under feet
...
# Linear Young's modulus profile
E_0 = 1. # unimportant
c = 2.
youngs_modulus_type = '$E = E_0(1 + (cz/f_r))$; $E_0$ = {0:.2f}, $c$ = {1:.2f}'.format(E_0, c)
youngs_modulus_expression = "E_0*(1. + c*(H/2. - x[2])/f_r)"
E = interpolate(Expression(youngs_modulus_expression,
                           f_mu=f_mu, E_0=E_0, c=c, f_r=lander_foot_radius,
                           H=H, degree=3), D)
...
func_lmda = Expression("f_lambda*"+youngs_modulus_expression,
                       f_lambda=f_lambda,
                       E_0=E_0, c=c, f_r=lander_foot_radius,
                       H=H, degree=3)
...
c = 1.
k = 0.1
youngs_modulus_type = '$E = E_0(1 + (cz/f_r))^k$; $E_0$ = {0:.2f}, $c$ = {1:.2f}, $k$ = {2:.2f}'.format(E_0, c, k)
youngs_modulus_expression = "E_0*pow(1 + c*(H/2. - x[2])/f_r, k)"
E = interpolate(Expression(youngs_modulus_expression,
                           c=c, k=k, E_0=E_0, f_r=lander_foot_radius, H=H, degree=3), D)

func_mu = Expression("f_mu*"+youngs_modulus_expression,
                     f_mu=f_mu, c=c, k=k, E_0=E_0, f_r=lander_foot_radius, H=H, degree=3)

func_lmda  = Expression("f_lambda*"+youngs_modulus_expression,
                        f_lambda=f_lambda,
                        c=c, k=k, E_0=E_0, f_r=lander_foot_radius, H=H, degree=3)
...
logz = np.log10(-u_norm[surface_mask,2])

ax[0].set_title(youngs_modulus_type)
ax[0].tricontour(x, y, logz, levels=14, linewidths=0.5, colors='k')
contour_function = ax[0].tricontourf(x, y, logz, levels=14, cmap=cc.cm.rainbow) #colorwheel

cbar = fig.colorbar(contour_function, ax=ax[0])
cbar.set_label('$\\log_{10} (w/w_{0})$', rotation=270, labelpad=16) # labelpad shifts the label right
...
fig.tight_layout()
fig.savefig('transfer_function_map.pdf')
